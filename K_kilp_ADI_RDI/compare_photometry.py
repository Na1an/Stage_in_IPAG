...
for i in range(len(ADI_res)):
    #data[i][0] = ADI_res[i]
    data[i][0] = RDI_flux_3_best[i]
    data[i][1] = RDI_flux_5_best[i]
    #data[i][3] = ADI_res_32[i]
    data[i][2] = RDI_flux_3_best_32[i]
    data[i][3] = RDI_flux_5_best_32[i]

data_SN = np.zeros((len(ADI_res), nb_data))
for i in range(len(ADI_res)):
    #data_SN[i][0] = ADI_SN[i]
    data_SN[i][0] = RDI_SN_3_best[i]
    data_SN[i][1] = RDI_SN_5_best[i]
    #data_SN[i][3] = ADI_SN_32[i]
    data_SN[i][2] = RDI_SN_3_best_32[i]
    data_SN[i][3] = RDI_SN_5_best_32[i]

data_total = pd.DataFrame(data[:,:], columns=['RDI_3_best_with_mask','RDI_5_best_with_mask','RDI_3_best_with_mask_32pxs','RDI_5_best_with_mask_32pxs'])
...
data_total_SN = pd.DataFrame(data_SN[:,:], columns=['RDI_SN_3_best_with_mask','RDI_SN_5_best_with_mask','RDI_SN_3_best_with_mask_32pxs','RDI_SN_5_best_with_mask_32pxs'])
