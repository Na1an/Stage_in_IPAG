...
if __name__ == "__main__":
    ...
    real_adi_contrast, real_adi_sn = get_contrast_and_SN("./ADI/real", positions[0], fwhm_for_snr, fwhm_flux[0]) 
    fake_comp_close_adi_contrast, fake_comp_close_adi_sn = get_contrast_and_SN("./ADI/fake_comp_close", positions[1], fwhm_for_snr, fwhm_flux[0])
    fake_comp_far_adi_contrast, fake_comp_far_adi_sn = get_contrast_and_SN("./ADI/fake_comp_far", positions[2], fwhm_for_snr, fwhm_flux[0])
    fake_disk_close_adi_contrast, fake_disk_close_adi_sn = get_contrast_and_SN("./ADI/fake_disk_close", positions[3], fwhm_for_snr, fwhm_flux[0])
    fake_disk_far_adi_contrast, fake_disk_far_adi_sn = get_contrast_and_SN("./ADI/fake_disk_far", positions[4], fwhm_for_snr, fwhm_flux[0])
    
    nb_data = 5
    l = len(real_adi_contrast)
    
    # contrast
    data = np.zeros((l, nb_data))
    for i in range(l):
        data[i, 0] = real_adi_contrast[i]
        data[i, 1] = fake_comp_close_adi_contrast[i]
        data[i, 2] = fake_comp_far_adi_contrast[i]
        data[i, 3] = fake_disk_close_adi_contrast[i]
        data[i, 4] = fake_disk_far_adi_contrast[i]

    # snr
    data_sn = np.zeros((l, nb_data))
    for i in range(l):
        data_sn[i, 0] = real_adi_sn[i]
        data_sn[i, 1] = fake_comp_close_adi_sn[i]
        data_sn[i, 2] = fake_comp_far_adi_sn[i]
        data_sn[i, 3] = fake_disk_close_adi_sn[i]
        data_sn[i, 4] = fake_disk_far_adi_sn[i]
    
    ##################
    # plot te result #
    ##################
    
    data_total = pd.DataFrame(data[:,:], columns=["real_adi", "fake_comp_close_adi", "fake_comp_far_adi", "fake_disk_close_adi", "fake_disk_far_adi"])
    ...
    plt.title("ADI - contrast of different companion", fontsize = 20)
    plt.xlabel("K_kilp", fontsize = "18")
    plt.ylabel("Contrast of the companion - diameter 4 px", fontsize = "18")
    #plt.ylim(0,70)
    plt.show()
    
    data_total_SN = pd.DataFrame(data_sn[:,:], columns=["real_adi", "fake_comp_close_adi", "fake_comp_far_adi", "fake_disk_close_adi", "fake_disk_far_adi"])
    ...
    plt.title("ADI - signal to noise ratio (S/N ratio) of different companion", fontsize=20)
    plt.xlabel("K_kilp", fontsize= "18")
