...
if __name__ == "__main__":
    ...
    path = "./spat_annular_mean/real/"
    # real, fake_comp, fake_disl
    real_3_best_contrast, real_3_best_sn = get_contrast_and_SN(path+"3_best", positions[0], fwhm_for_snr, fwhm_flux[0]) 
    real_5_best_contrast, real_5_best_sn = get_contrast_and_SN(path+"5_best", positions[0], fwhm_for_snr, fwhm_flux[0])
    real_7_best_contrast, real_7_best_sn = get_contrast_and_SN(path+"7_best", positions[0], fwhm_for_snr, fwhm_flux[0])
    real_9_best_contrast, real_9_best_sn = get_contrast_and_SN(path+"9_best", positions[0], fwhm_for_snr, fwhm_flux[0])
    real_11_best_contrast, real_11_best_sn = get_contrast_and_SN(path+"11_best", positions[0], fwhm_for_snr, fwhm_flux[0])

    nb_data = 5
    l = len(real_3_best_contrast)
    
    # contrast
    data = np.zeros((l, nb_data))
    for i in range(l):
        data[i, 0] = real_3_best_contrast[i]
    ...
    data_total = pd.DataFrame(data[:,:], columns=["real_3_best_ref", "real_5_best_ref","real_7_best_ref", "real_9_best_ref","real_11_best_ref"])
    ...
    plt.title("spat-annular-mean - real - contrast of different companion", fontsize = 20)
    plt.xlabel("K_kilp", fontsize = "18")
    plt.ylabel("Contrast - diameter 4 px", fontsize = "18")
    #plt.ylim(0,70)
    plt.show()
    
    data_total_SN = pd.DataFrame(data_sn[:,:], columns=["real_3_best_ref", "real_5_best_ref","real_7_best_ref", "real_9_best_ref","real_11_best_ref"])
    ...
    plt.title("spat-annular-mean - real - signal to noise ratio (S/N ratio) of different companion", fontsize=20)
    plt.xlabel("K_kilp", fontsize= "18")
